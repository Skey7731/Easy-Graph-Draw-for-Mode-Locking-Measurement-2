...
class GraphESA(tk.Frame):
    def __init__(self, master=None):
        ...
        #-----------------------------------------------
        # データ保存用
        self.file_data = fileData.FileData(['csv','set'])
        ...
        self.CH_button3 = ttk.Radiobutton(CH_frame, variable=self.CH_state, value='3', text='ave', command=self.draw_graph)

        # これの配置
        self.CH_button1.pack(side='left')
        self.CH_button2.pack(side='left')
        self.CH_button3.pack(side='left')
        CH_frame.pack()
        ...
        # グラフタイトル設定
        self.ax.set_title('RF Spectrum', fontsize=22, visible=True)
        
        # 軸ラベル設定
        self.ax.set_xlabel('Freqency / MHz', visible=True)
    ...
    def draw_graph(self):
        ...
        if self.file_data.y1 and (not self.file_data.y2):
            ...
            self.CH_button3['state'] = 'disabled'
        else:
            self.CH_button1['state'] = 'abled'
            self.CH_button2['state'] = 'abled'
            self.CH_button3['state'] = 'abled'

        # xの設定
        self.x = [i * 1e-6 for i in self.file_data.x]
        self.ax.set_xlabel('Freqency / MHz', visible=True)
        if len(str(int(self.x[-1]))) >= 4:
            self.x = [i * 1e-9 for i in self.file_data.x]
            self.ax.set_xlabel('Freqency / GHz', visible=True)

        # yの設定
        if self.CH_state.get() == 1:
            self.y = copy.copy(self.file_data.y1)
        elif self.CH_state.get() == 2:
            self.y = copy.copy(self.file_data.y2)
        else:
            self.y = [(self.file_data.y1[i] + self.file_data.y2[i]) / 2 for i in range(len(self.x))]
        ...
        # 最大値を取る周波数
        peak_freq = self.x[self.y.index(max(self.y))]

        self.file_text.set(self.file_data.file_name + '\n最大強度を取る周波数: ' + str(round(peak_freq, 3)))


        # ウィンドウタイトルの設定
        self.master.title('ESA ' + self.file_data.file_path)

    # ファイルドロップ時の処理
    def process_droped_file(self, event):
        self.file_data.dnd_get_path(event)  # ファイルの名前やパスを取得
        self.file_data.read_ESA()   # 波形のcsvファイルとして読み込み
        self.draw_graph()                   # 画面内に新しいグラフの描画

    # ファイルの選択
    def file_select(self):
        file = tk.filedialog.askopenfilename(title='ファイルを選択：ESA', filetypes=[('csvファイル', 'csv'),('setファイル', 'set')])
        if not file:
            return
        self.file_data.set_file_data(file)
        self.file_data.read_ESA()
        self.draw_graph()
